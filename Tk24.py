...
def click(event):
    global scvalue
    text = event.widget.cget("text")     #to get text of button which's pressed .Yes,here unlike in entry widgets we cant just have a var.get()
    if text == "=":
        if scvalue.get().isdigit():
            ...
            except Exception as e:  #if we enter something which cant be calculate for e.g.:2/ then an exceptions can be raised
                print(e)
                value = "Error"
        scvalue.set(value)
        screen.update()
        ...
        scvalue.set("")
        screen.update()
    else:                 #this will show numbers on buttons which we'd have pressed like for 78 we'd have to press 7 then 8 so tht appending's done here
        scvalue.set(scvalue.get() + text)
        screen.update()

root = Tk()
root.geometry("644x670")
...
b = Button(f, text="9", padx=28, pady=18,height=1,font="lucida 35 bold")
